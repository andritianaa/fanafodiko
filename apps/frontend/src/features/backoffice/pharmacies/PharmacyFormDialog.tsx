...
import { useCreatePharmacy, useUpdatePharmacy } from '@/features/pharmacy/api/hooks';
...
export function PharmacyFormDialog({ open, onClose, pharmacy }: Props) {
  ...
  const onSubmit = (data: CreatePharmacyInput) => {
    const payload = { ...data, openingHours: isOpen24h ? [] : data.openingHours };
    if (isEdit && pharmacy) {
      update(
        { id: pharmacy.id, data: payload },
        {
          onSuccess: () => { toast.success('Pharmacie mise à jour'); onClose(); },
          onError: () => toast.error('Erreur lors de la mise à jour'),
        }
      );
    } else {
      create(payload, {
        onSuccess: () => { toast.success('Pharmacie créée'); onClose(); },
      ...
      <DialogContent className="!max-w-4xl w-full h-[88vh] flex flex-col p-0 overflow-hidden">
        ...
        >
          <Tabs defaultValue="info" className="flex-1 flex flex-col min-h-0 px-6 pt-4">
            ...
            {/* ─── Infos générales ─── */}
            <TabsContent value="info" className="flex-1 overflow-y-auto space-y-3 pr-1">
              <div>
                <Label>Nom *</Label>
                <Input
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="15 Rue de l'Indépendance"
                />
              </div>
            ...
            <TabsContent value="location" className="flex-1 overflow-y-auto space-y-3 pr-1">
              <Controller
                control={control}
                name="coordinates"
                render={({ field }) => (
            ...
            {/* ─── Contacts multiples ─── */}
            <TabsContent value="contacts" className="flex-1 overflow-y-auto pr-1">
            ...
            <TabsContent value="hours" className="flex-1 overflow-y-auto pr-1">
              <p className="text-xs text-muted-foreground mb-3">
                Activez/désactivez chaque jour et saisissez les horaires d'ouverture.
              </p>
              <Controller
                control={control}
                name="openingHours"
